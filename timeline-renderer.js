...
(function (root) {
  ...
  function routineRuntime(makeCore, makeRunCore) {
    ...
    function layout() {
      ...
      // Lanes Fill: the largest scale at which the routine fits the view,
      // drawn whole; a routine too long for it keeps 12 px a minute and its
      // breaks, and scrolls.
      const room = fillRoom(info, cols),
        g = room ? core.fit(items, sections, needs, { liveRest }, room) : core.layout(items, sections, needs, { liveRest });
      fitted = !!g.fits;
      for (const el of els) {
  ...
  function routineCore() {
    ...
    // Lanes Fill (§9): the largest scale whose sheet is no taller than
    // `height` px, drawn without breaks, found by bisection (8 layouts at
    // most, all arithmetic). A routine too long for the screen even at the
    // smallest scale falls back to the plain 12 px a minute with long steps
    // broken, and scrolls (`fits: false`).
    function fit(items, sections, needs, options, height) {
      const at = (scale) => layout(items, sections, needs, { ...options, scale, breaks: false }),
        plain = () => ({ ...layout(items, sections, needs, options), fits: false });
      const end = Math.max(0, ...items.map((i) => i.until ?? i.at));
      if (!(height > 0) || !end) return plain();
      let best = at(SCALE.min);
      if (best.height > height) return plain();
      ...
      let lo = SCALE.min,
        hi = Math.min(SCALE.max, Math.max(SCALE.min, height / (end / 60)));
      const top = at(hi);
      if (top.height <= height) return { ...top, fits: true };
