...
(function (root) {
  ...
  function routineRuntime(makeCore) {
    ...
    function tick(fromLayout) {
      ...
      // live card shows its whole description): lay out again now, before
      // following the run, so the scroll below uses the new positions. (The
      // toggle event lays out too, but only after this has scrolled; the
      // card that just finished folding its notes would then pull the new
      // live card up under the header.)
      ...
      // so the page can still be scrolled by hand in between. The top live
      // card (or, in a rest or a gap, the rest and the move up next) sits
      // just under the run header; with nothing live, the now line does.
      ...
      const tops = liveSteps.map((el) => parseFloat(el.style.top)).concat(upcoming ? [parseFloat(upcoming.style.top)] : []),
        target = core.followTop(tops, y),
        base = lanes.getBoundingClientRect().top + window.scrollY;
      window.scrollTo({ top: Math.max(0, base + target - headerInset(run) - 12), behavior: "smooth" });
    }
...
.run-controls>.edit-pill,.run-controls>.edit-toggle,.run-done-actions>.edit-pill,.run-done-actions>.edit-toggle{position:static!important;display:inline-flex!important;align-items:center;justify-content:center;flex:none;height:44px;margin:0!important;padding:0 14px!important;border:0!important;border-radius:999px!important;background:#24313a!important;color:#e9f1f5!important;font:700 14px/1 system-ui,-apple-system,'Segoe UI',Roboto,sans-serif!important;opacity:1!important;box-shadow:none!important;-webkit-backdrop-filter:none!important;backdrop-filter:none!important;text-decoration:none!important;white-space:nowrap;cursor:pointer}
...
@media(max-width:699px),(max-height:699px){.run-photo{display:none!important}}
...
.run-detail{margin:8px 0 0;font-size:15px;line-height:1.3;color:#cfdde5;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
...
.run-toggles{display:flex;gap:8px;max-width:440px;margin:6px auto 8px}
.run-tg{display:flex;flex:1 1 0;align-items:center;gap:8px;height:40px;padding:0 8px 0 10px;border:1px solid #2b3740!important;border-radius:12px;background:#1c2830;font-size:14px;white-space:nowrap}
...
@media(max-width:420px){.run-step{font-size:22px}.run-secs{font-size:38px}.run-controls{gap:8px}.run-pause{min-width:100px}.run-tg{font-size:13px;gap:6px}}`;
    ...
    const $ = (root, selector) => root.querySelector(selector),
      ui = {
        ...
        doneActions: $(top, ".run-done-actions"),
    ...
    // The page's ✎ Edit button (the studio passes its own; view.html adds
    // an .edit-pill) never floats over the steps: in the run bar while a
    // run shows, beside Restart and Close when it is done, and back where
    // the page put it otherwise (the page hides it while the ready card's
    // own Edit is on screen, and tucks it beside the docked Start).
    let pillHome = null;
    function placePill(mode) {
      const pill = host ? host.edit : doc.querySelector(".edit-pill");
      if (!pill) return;
      const slot = mode === "run" ? ui.controls : mode === "done" ? ui.doneActions : null;
      if (slot) {
        if (pill.parentNode === slot) return;
        if (!pillHome) pillHome = { parent: pill.parentNode, next: pill.nextSibling };
        slot.append(pill);
      } else if (pillHome) {
        const { parent, next } = pillHome;
        pillHome = null;
        if (parent) parent.insertBefore(pill, next && next.parentNode === parent ? next : null);
      }
    }
    function show(mode) {
      placePill(mode);
    ...
    function running(state, e, total) {
      ...
      // The header stays compact: one line of the lead step's description.
      // Its notes and the whole description are in the live card on the
      // sheet, which opens while it runs, so nothing is shown twice.
      ...
      // The lead step's picture, large where there is room (a wide screen;
      // CSS hides it on a phone); the next step's as a thumbnail.
    ...
    return {
      ...
      destroy() {
        ...
        sheet({ active: false });
        placePill("idle");
        doc.body.classList.remove("run-active", "run-docked", "run-routine");
