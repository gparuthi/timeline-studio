...
(function (root) {
  ...
  function parse(text) {
    ...
    text.split(/\r?\n/).forEach((raw, index) => {
      ...
      if (/^\d{1,2}(?::\d{2})?\s*(?:AM|PM)?\s*\|/i.test(s)) {
        const fields = s
          .split(/(?<!\\)\|/)
        ...
        const [clock, title, detail = "", icon = "pin", color = "sky"] = fields;
    ...
    for (const day of model.days) {
      ...
      day.end = day.range?.[1] ?? Math.ceil(day.events.at(-1).minutes / 60) * 60;
      if (day.end === day.start) day.end = day.start + 60;
      day.iso = isoDate(day.label || model.date);
      for (const event of day.events) {
        if (event.minutes < day.start || event.minutes > day.end)
          throw new Error(
  ...
  function layoutRuntime() {
    ...
    function layoutOne(timeline) {
      ...
      cards.forEach((card, i) => {
        const y = centers[i] + topPad,
          ...
          x = card.offsetLeft;
        card.style.top = y + "px";
        connectors += `<path class="connector" d="M${axis},${anchor} C${axis + 12},${anchor} ${x - 12},${y} ${x},${y}"/><circle class="event-dot" cx="${axis}" cy="${anchor}" r="${axis < 100 ? 4 : 6}"/>`;
      });
    ...
    function tickOne(timeline, ymd, minutesNow) {
      const cards = [...timeline.querySelectorAll(".event")],
        ...
        shapes = timeline.querySelector(".connectors").children;
      let next = -1;
      cards.forEach((card, i) => {
        const minute = Number(card.dataset.minute),
          past = isOver || (isToday && minute < now);
        card.classList.toggle("past", past);
        shapes[2 * i]?.classList.toggle("past", past);
        shapes[2 * i + 1]?.classList.toggle("past", past);
    ...
    const days = m.days
      .map((day, index) => {
        ...
        const events = day.events
          .map(
            (e) =>
              `<li class="event ${e.color}" data-minute="${e.minutes - day.start}" style="--m:${e.minutes - day.start}"><time>${label(e.minutes)}</time><div class="event-body"><span class="event-icon" aria-hidden="true">${e.icon.startsWith("@") ? `<img src="${escape(m.assets[e.icon.slice(1)])}" alt="">` : `<svg viewBox="0 0 40 40"><use href="#${e.icon}"/></svg>`}</span><h2>${escape(e.title)}</h2>${e.detail ? `<p>${escape(e.detail)}</p>` : ""}</div></li>`,
          )
          .join("");
        const heading =
          m.days.length > 1
            ? `<div class="day-heading" role="heading" aria-level="2">${escape(day.label || day.iso || `Day ${index + 1}`)}</div>`
            : "";
...
${extraCss}
${themes[m.theme]}</style></head><body class="theme-${m.theme}">${art.symbols}<main class="sheet"><header class="masthead">${decorations}<h1>${escape(m.title)}</h1>${date ? `<p class="date">${escape(date)}</p>` : ""}${m.subtitle ? `<p class="tagline">${escape(m.subtitle)}</p>` : ""}</header>${days}${notesHtml(m.notes)}${m.footer ? `<footer><span>${escape(m.footer)}</span></footer>` : ""}</main><script>${runtime}<\/script></body></html>`;
  }
  ...
  const api = { parse, render, icons, themes: themeNames, encodeLink, decodeLink };
