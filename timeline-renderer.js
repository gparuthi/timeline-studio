...
(function (root) {
  ...
  function routineRuntime(makeCore, makeRunCore) {
    ...
    function tick(fromLayout) {
      ...
      // live card fills up to it. Grey, and "paused", while paused.
      ...
      const tops = liveSteps.map((el) => parseFloat(el.style.top)).concat(upcoming ? [parseFloat(upcoming.style.top)] : []),
        target = cols ? y - (run.view || window.innerHeight) / 3 : core.followTop(tops, y) - headerInset(run) - 12,
  ...
  function routineCore() {
    ...
    return { chip, atLabel, sectionLength, atOnce, summary, chips, lanes, layout, yAt, followTop, MIN, GAP };
  }
...
@media(max-width:359px){.rv-ct{min-width:56px;font-size:26px}.rv-live,.rv-done{gap:6px}}`;
    ...
    const measure = () => {
      ...
      doc.documentElement.style.setProperty("--run-col", width > 1100 ? "400px" : "360px");
      if (next !== cols) {
        cols = next;
        if (alive) render();
      }
    ...
    const sheet = (next) => {
      ...
      const sig = next.active ? "" : JSON.stringify([next.cols, next.view, beeps, voice, next.finished || null]);
    ...
    function ready() {
      ...
      setHtml(
        ui.chips,
        chips.map((c) => `<button type="button" class="rc-chip" data-chip="${esc(c)}" aria-pressed="false"><span class="rc-ck" aria-hidden="true">${icons.check}</span>${esc(show(c))}</button>`).join(""),
      );
