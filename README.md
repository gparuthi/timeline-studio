...
Once you edit, the studio's own address bar carries the timeline (`index.html#z=…`, updated as you type via `history.replaceState`; deflate-compressed, base64url, or `#t=…` uncompressed in browsers without CompressionStream), so copying the browser URL is enough to share; whoever opens it sees the timeline with the editor tucked away. `view.html#z=…` is the same fragment on a bare viewer page. Nothing in these long links is sent to a server: the fragment never leaves the browser. Embedded images inflate them; past roughly 30k characters some browsers refuse them, so prefer **Export → Timeline (HTML)** for image-heavy days.

## Short links

**Export → Copy link** copies a short link. It posts the same compressed payload the long link carries to a small Cloudflare Worker (`worker/`, served at `https://tl.gaup.uk`) and copies back `https://tl.gaup.uk/<id>`; if the text has a `link:` line it copies that named link instead (below). If the link server cannot be reached, the long fragment link is copied so the button always yields something that opens. The id is a prefix of the payload's SHA-256, so the same timeline always gets the same link. `GET /<id>` serves the viewer with the timeline inlined (plus `og:title` for message previews), `GET /<id>.txt` returns the text, and a tap on any card or the ✎ Edit pill opens the studio. This is the one feature that sends the timeline off your device: the text sits in Workers KV with no expiry. Deploy the Worker with `cd worker && npx -y wrangler@latest deploy` (it needs a `wrangler login` with Workers KV access on the `gaup.uk` account; the KV namespace id is in `wrangler.jsonc`). For local work, run `npx wrangler dev --local --ip 0.0.0.0` in `worker/` and set `localStorage.shortLinkApi = "http://<lan-ip>:8787/"` in the studio.

### Named links that follow your edits

Add `link: la-week` to the text (or use **Export → Custom link…**, which asks for a name and writes the line for you) and the studio publishes every edit to `https://tl.gaup.uk/la-week` about a second and a half after you stop typing; the address never changes, so people you already sent it to always see the latest version. The first publish claims the name and the Worker returns an edit id, which the studio appends to the line: `link: la-week k3x9…`. The text is then the proof of ownership: paste it into the studio on any device and that device can update the name too. Anything without the id (or with a wrong one) is told the name is taken and **Copy link** falls back to a plain content link. The Worker strips the id from every payload before storing it, so the short link, the viewer, the `.txt` form and the "Edit" hand-off never carry it; only your own text, address bar and saved studio copies do, so share the short link rather than the browser URL. The pane shows the link's state under the toolbar at all times ("Linked to la-week", "Saving…", or a red warning that the edits are not on the link, which you tap to fix: paste the id, publish your version over one changed elsewhere, or retry), and **Done** publishes at once and reports. An id that arrives in the text is kept by that browser, so later opens without it still publish. Names are 3–32 lowercase letters, digits or hyphens. Named links are served with `Cache-Control: no-store`; content links stay cacheable because they never change.

## Calendar feed

**Export → Calendar feed** copies the same link as Copy link with `.ics` on the end (`https://tl.gaup.uk/la-week.ics` for a named link, `https://tl.gaup.uk/<id>.ics` otherwise). Subscribe to it in Google Calendar (Other calendars → **From URL**) or in iOS Calendar (Add Subscription Calendar) and every event of every dated day appears as a calendar event; a named feed follows your edits. The feed is generated by the Worker from the text on each request: each event is a UTC instant converted from the written time in the timeline's `timezone:` (a day's own `timezone:` wins; Pacific when none is given), because Google Calendar reads zone-less times as UTC; a span keeps its end, any other event lasts an hour or until the next one, the description becomes the event description, the place before a map link becomes the location, and the link becomes the event URL. Undated days are skipped. Google polls subscribed URLs on its own schedule, typically every several hours and up to a day; iOS lets you choose the refresh interval in the calendar's settings, so the phone is the faster way to follow edits. Import the file once instead if you never want it to change.

## Change it in plain words

The box at the top of the editor pane takes an instruction instead of an edit: `move golf to 3 pm`, `add lunch with Priya at 12:30 on Wednesday at the airport`, `push everything on Monday back by one hour`, `I'll skip SF on Thursday`. The studio posts the text and the instruction to the link server (`POST /command` with `{ text, command, today }`), which asks Workers AI (`@cf/qwen/qwen3-30b-a3b-fp8`, reasoning on) for the complete edited text and returns `{ text, note }`. The whole document goes both ways because a timeline is a few hundred tokens, and a full text is far easier to validate than a diff: the studio parses the answer, applies it only if it is still a timeline, puts the cursor on the first changed line, and shows "N lines changed · Undo" (the previous text is kept; on a Mac, Cmd+Z works too). A question or an instruction that cannot be applied comes back unchanged with the model's note in the status line. "Today" and "tomorrow" resolve against the date the studio sends, and a bare time follows the event (dinner at 7:30 is 19:30). Embedded images and the `link:` edit id are stripped before the model sees the text and put back afterwards. A command costs about 25 neurons against Workers AI's free 10,000 a day, takes 3–13 seconds, and the Worker rate-limits it to 20 a minute per address so a stranger cannot spend the allowance. Like short links, this sends the timeline off your device; nothing is stored. Local dev: `wrangler dev` (without `--local`) runs the AI binding remotely.
