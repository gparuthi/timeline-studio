...
Supported settings: `title`, `date` (display text), `subtitle`, `range` (per day), `theme`, `footer`, repeatable `note`, repeatable `day`, `header-art`, `timezone` (`timezone: PT`, `ET`, `CET`, `IST`, or an IANA name such as `Asia/Tokyo`; written before any `day:` it covers the whole timeline, under a `day:` it covers that day, for trips that cross zones), and `city` (`city: Los Angeles`, added to place searches, see below; global or per day like `timezone`). The sheet always shows the times as written; the time zone matters to the calendar feed. Whole-line comments start with `#`.
