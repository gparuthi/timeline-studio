...
The run is a small state object (`startedAt`, `pausedAt`, `pausedMs`, `shiftMs`) from which elapsed time is always computed, never counted up, so a locked phone, a background tab or a reload resumes at the right second. An edit during a run takes effect at once and keeps the elapsed time.

**Synced runs.** On a named link (`tl.gaup.uk/<name>`) the run is shared: start it on the phone and the TV with the same link open switches into run mode; pause, skip, back and stop on any device act on all of them, and a page opened or reloaded mid-run joins at the right second. The state lives in a Durable Object per name (`RunRoom`, `worker/src/runroom.js`, SQLite-backed, WebSockets through the hibernation API): `GET /run/<name>` is the socket (the server sends `{ state, now }` on connect and after every change; pages send `{ op: "start" | "pause" | "resume" | "seek" | "stop", shiftMs }`), `GET /run/<name>.json` answers `{ state, now }` for a page whose socket is down (it polls every 3 s and reconnects with backoff), and `POST /run/<name>` takes an op from such a page. The server's clock is the run's clock: each page pings, and estimates its skew from the quickest round trip, so devices whose clocks disagree still agree on elapsed time; a socket that stops answering pings is dropped and reopened. A page connects only while its text is a routine. Unnamed pages and standalone copies (GitHub Pages, `#z=` links, saved files, `view.html`) run locally, with the state in localStorage under a hash of the steps, so a reload resumes there too. In the studio the run lives in the studio page rather than the preview frame, because audio, the wake lock and vibration need the top-level page, and its focus panel and controls sit outside the preview's scroller so they stay put on iOS; the frame is sent the run's position for the sheet. Sound needs a tap on iOS: Start, Resume and the sound button unlock it, and a run resumed by a reload shows **Tap for sound**. Verify on an iPhone: beeps and voice after the tap, the screen staying on, vibration (not on iOS Safari).
