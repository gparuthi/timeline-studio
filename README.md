...
**Run mode.** Start (the card's, the docked one, in the studio, `view.html` and exported HTML) pins a focus panel over the sheet. The step that ends soonest drives it: its title, a countdown bar, `4:00 left of 10 min` and one line of its description (and its picture, large, on a screen at least 700 px wide and tall); any other live step gets a smaller row of its own with a bar and countdown, and the next step is listed underneath. The panel stays compact: the step's notes and its whole description are in its card on the sheet, which opens while it runs (description, then notes), so nothing is shown twice. During a rest the panel counts down "then Side plank L" and shows that move large with its picture. On the sheet finished steps fade, live ones get a green outline and fill up to a "now" line that runs across the lanes behind the cards (so it shows in the gutter and between cards, never through a card's text), a live rest grows to show its countdown, and the page scrolls once each time the live steps change, putting the top of the highest live card (or a live rest and the move after it) just under the panel. The panel's height is measured at that moment, since it changes with the step, and the cards are laid out again first, so a finished card folding its notes cannot pull the new one up under the panel. The bottom bar shows `elapsed 6:00 · left 39:00 · done ≈ 6:15 PM` (the one clock time, and only once a run is going), two labelled switches of equal width, **Beeps** and **Announce steps** (the pair fills the row on a phone and stays 440 px wide, centred, on a wider screen), each remembered per device (the old cycling sound setting carries over: beeps + voice turns both on, off turns both off), then Back (to the step's start, or the previous step within 3 s of it), Pause/Resume, Skip and Stop, which asks first. Cues: short beeps at 3, 2, 1 s before a step ends and a long beep at each new step (Beeps); "Side plank left, 45 seconds" as each step starts (Announce steps; when two steps are live, the one that started); a vibration at each new step. The screen stays awake while running. At the end: `Done · 10:14` (the time it really took) and Restart. The run is a small state (`startedAt`, `pausedAt`, `pausedMs`, `shiftMs`) from which elapsed time is always computed, never counted, so a locked phone, a background tab or a reload resumes at the right second, and an edit during a run takes effect at once without losing the elapsed time.
